...
EOF
    ...
    while true; do
        read -p "👉 Thêm thiết bị vào hệ thống? (y/n): " add_device
        [[ ! "$add_device" =~ ^[Yy]$ ]] && break

        echo -e "\n  ${BLUE}1)${NC} 📷 Camera (CCTV)"
        echo -e "  ${BLUE}2)${NC} 🔊 Loa báo động"
        echo -e "  ${BLUE}3)${NC} ❄️  Smart Home (Tuya/Sonoff)"
        read -p "👉 Chọn loại (1-3): " dev_type

        if [[ "$dev_type" == "1" ]]; then
            echo -e "\n${CYAN}--- CẤU HÌNH CAMERA ---${NC}"
            echo -e "  1) Hikvision  2) Dahua/Kbvision  3) Ezviz/Imou  4) RTSP thủ công"
            read -p "👉 Hãng camera: " cam_brand

            if [[ "$cam_brand" == "4" ]]; then
                read -p "Nhập link RTSP: " cam_rtsp
            else
                read -p "IP Camera: " cam_ip
                read -p "Username (mặc định admin): " cam_user
                cam_user="${cam_user:-admin}"
                read -p "Password / Verification Code: " cam_pass
                case "$cam_brand" in
            ...
            read -p "Tên camera (VD: CAM_KHO_01): " cam_name
            ...
            read -p "IP Loa: " speaker_ip
            read -p "Tên Loa (VD: SPEAKER_01): " speaker_name
